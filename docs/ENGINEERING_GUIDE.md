...
- Add browser checks over `/artifacts/{artifactId}` using `agent-browser` or OpenAI Browser Use for narration, music, captions, breath, meditation, image/photo, cinema when enabled, companion rail, and completion POST.
...
**Status:** Partially implemented. Backend planning, renderer contracts, provider smoke, local handoff, fixture browser playback through `agent-browser` or OpenAI Browser Use, quiet completion capture, companion scaffolding, and a no-camera live shell exist; production Hermes-agent routing/subscription, scheduled acceptance E2E, provider-backed browser checks, and sensor guidance remain open.
