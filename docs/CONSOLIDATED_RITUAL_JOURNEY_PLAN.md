...
5. Journey CLI integration
   ...
   - Reports include selected tool sequence, requested surfaces, render policy, artifact URL, manifest surfaces, and browser check result.
...
- Browser checks over provider-backed artifacts, not only fixture manifests.
...
Context:
Phases 1-6 are focused on ritual contract, Hermes-agent tool choice, scheduled invitation, renderer/provider hardening, browser playback E2E, and Journey CLI proof. Your task is to produce a detailed implementation plan for Phases 7-9:
...
- Browser E2E acceptance criteria for the designer-facing experience.
