...
def derive_runtime_method_state_policy(
    ...
    policy: RuntimeMethodStatePolicy = {
        "depthLevel": "gentle",
        "blockedMoves": ["active_imagination"],
        "preferredMoves": [],
        ...
        "practiceConstraints": {},
        "reasons": ["method_context_missing_or_stale_default_to_gentle"],
    }
    ...
    if not isinstance(method_state, dict):
        return policy
    adaptation_profile = method_context.get("adaptationProfile")
    ...
    practice_preferences = _adaptation_scope_dict(
        adaptation_profile,
        explicit_scope="practice",
        learned_scope="practicePolicy",
    )
    ...
    if rejected_patterns:
        policy["avoidPhrasingPatterns"] = rejected_patterns[:5]
    ...
    if communication_tone in {"gentle", "direct", "spacious"}:
    ...
    elif depth_preference == "deep_amplification":
        policy["preferredMoves"] = list(
            dict.fromkeys([*policy["preferredMoves"], "deepen_when_held"])
        )
        policy["reasons"].append("Interpretation preference leaves room for deeper amplification.")
    ...
    if isinstance(containment, dict):
        grounding_need = str(containment.get("groundingNeed") or "").strip()
        if grounding_need == "grounding_first":
            ...
            policy["practiceConstraints"] = {"requireGroundingCompatible": True}
            ...
            policy["practiceConstraints"] = {"preferLowIntensity": True}
    ...
    if isinstance(relational_field, dict):
        ...
        if relationship_contact in {"thin", "isolated"} or isolation_risk in {"moderate", "high"}:
            policy["preferredClarificationTargets"] = list(
                dict.fromkeys(
                    [
            ...
            policy["reasons"].append("Relational field suggests starting with support and contact.")
    ...
    if isinstance(practice_loop, dict):
        ...
        if recent_outcome_trend == "activating":
            policy["reasons"].append("Recent practice outcomes suggest keeping work low intensity.")
            if policy["depthLevel"] == "standard":
                policy["depthLevel"] = "gentle"
    ...
    if practice_preferences:
        preferred_modalities = _string_list(practice_preferences.get("preferredModalities"))
        avoided_modalities = _string_list(practice_preferences.get("avoidedModalities"))
        if preferred_modalities:
            policy["practiceConstraints"] = {
                **policy["practiceConstraints"],
                "preferredModalities": preferred_modalities,
        ...
        practice_max_duration = practice_preferences.get("maxDurationMinutes")
        if isinstance(practice_max_duration, int) and practice_max_duration > 0:
            policy["practiceConstraints"] = {
                **policy["practiceConstraints"],
                "maxDurationMinutes": practice_max_duration,
            }
