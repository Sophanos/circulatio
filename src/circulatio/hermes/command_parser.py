...
class CirculatioCommandParser:
    ...
    def _parse_journey(self, tokens: list[str]) -> ParsedCirculationCommand:
        ...
        if tokens[1] == "experiment":
            return self._parse_journey_experiment(tokens)
        if tokens[1] == "pause":
    ...
    def _parse_journey_experiment(self, tokens: list[str]) -> ParsedCirculationCommand:
        if len(tokens) < 3:
            raise ValidationError("/circulation journey experiment requires an action.")
        action = tokens[2]
        if action == "list":
            return self._parse_journey_experiment_list(tokens)
        if action == "get":
            return self._parse_journey_experiment_get(tokens)
        if action == "start":
            return self._parse_journey_experiment_start(tokens)
        if action in {"quiet", "resume", "complete", "release", "archive"}:
            return self._parse_journey_experiment_respond(tokens, action=action)
        raise ValidationError(f"Unknown /circulation journey experiment action: {action}")

    def _parse_journey_experiment_list(self, tokens: list[str]) -> ParsedCirculationCommand:
        payload: dict[str, object] = {}
        statuses: list[str] = []
        index = 3
        while index < len(tokens):
            token = tokens[index]
            if token == "--journey-id":
                payload["journeyId"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--journey-label":
                payload["journeyLabel"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--status":
                statuses.append(self._require_value(tokens, index, token))
            ...
            if token == "--limit":
                ...
                index += 2
                continue
            raise ValidationError(f"Unknown /circulation journey experiment list option: {token}")
        if statuses:
            payload["statuses"] = statuses
        return ParsedCirculationCommand(
            operation="circulatio.journey.experiment.list",
            payload=payload,
            subcommand="journey experiment list",
        )

    def _parse_journey_experiment_get(self, tokens: list[str]) -> ParsedCirculationCommand:
        payload, index = self._parse_experiment_reference(
            tokens,
            start_index=3,
            subcommand="journey experiment get",
            allow_include_deleted=True,
        )
        if index != len(tokens):
            raise ValidationError(
                f"Unknown /circulation journey experiment get option: {tokens[index]}"
            )
        return ParsedCirculationCommand(
    ...
    def _parse_journey_experiment_start(self, tokens: list[str]) -> ParsedCirculationCommand:
        payload = self._parse_window_flags(tokens[3:])
        index = 3
        while index < len(tokens):
            token = tokens[index]
            if token in {"--window-start", "--window-end"}:
                ...
                continue
            if token == "--brief-id":
                payload["briefId"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--journey-id":
                payload["journeyId"] = self._require_value(tokens, index, token)
            ...
            if token == "--journey-label":
                ...
                continue
            if token == "--surface":
                payload["surface"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--title":
                payload["title"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--summary":
                payload["summary"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--source":
                ...
                index += 2
                continue
            if token == "--preferred-move":
                payload["preferredMoveKind"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--question":
                payload["currentQuestion"] = self._require_value(tokens, index, token)
            ...
            if token == "--action-text":
                payload["suggestedActionText"] = self._require_value(tokens, index, token)
                index += 2
                continue
            if token == "--body-first":
                payload["bodyFirst"] = True
                index += 1
                continue
        ...
        return ParsedCirculationCommand(
            ...
            payload=payload,
            subcommand="journey experiment start",
        )

    def _parse_journey_experiment_respond(
        self,
        tokens: list[str],
        *,
        action: str,
        ...
        payload, index = self._parse_experiment_reference(
            tokens,
            start_index=3,
            subcommand=f"journey experiment {action}",
        )
        payload["action"] = action
        while index < len(tokens):
            token = tokens[index]
            if token == "--next-check-in-due-at":
                payload["nextCheckInDueAt"] = self._require_value(tokens, index, token)
                index += 2
                continue
            raise ValidationError(
                f"Unknown /circulation journey experiment {action} option: {token}"
            )
        return ParsedCirculationCommand(
    ...
    def _parse_experiment_reference(
        self,
        tokens: list[str],
        *,
        start_index: int,
        ...
        while index < len(tokens):
            ...
            if token == "--experiment-id":
                payload["experimentId"] = self._require_value(tokens, index, token)
                index += 2
                continue
            ...
            if "experimentId" in payload:
                raise ValidationError(
                    f"/circulation {subcommand} accepts only one experiment reference."
                )
            payload["experimentId"] = token
            index += 1
        if "experimentId" not in payload:
            raise ValidationError(f"/circulation {subcommand} requires an experiment id.")
