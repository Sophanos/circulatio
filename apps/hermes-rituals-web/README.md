...
# start the app, then drive it with agent-browser or OpenAI Browser Use
bun run dev
agent-browser open http://127.0.0.1:3000/artifacts/{artifactId}
agent-browser snapshot -i
# or use OpenAI Browser Use in Codex to open the same /artifacts/{artifactId} URL
```
...
- Browser verification may use `agent-browser` (https://github.com/vercel-labs/agent-browser) or OpenAI Browser Use in the Codex in-app browser. Start the local app, open `/artifacts/{artifactId}`, verify narration/music/cinema/breath/body completion behavior, and confirm a narrow breath+music artifact does not show narration, transcript, or cinema UI.
